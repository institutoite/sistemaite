    ...
    <div class="row">
        ...
        <div class="col-xs-12 col-sm-12 col-md-6 col-lg-4" >
            <div class="form-floating mb-3 text-gray">
                <select class="form-control @error('mododocente_id') is-invalid @enderror" data-old="{{ old('mododocente_id') }}" name="mododocente_id" id="country">
                    <option value="1" selected> Bolivia</option>
                    @foreach ($mododocentes as $modo)
                        @isset($docente)     
                            <option  value="{{$modo->id}}" {{$modo->id==$docente->modo_id ? 'selected':''}}>{{$modo->mododocente}}</option>     
                        @else
        ...
        <div class="col-xs-12 col-sm-12 col-md-6 col-lg-4" >
            <div class="form-floating mb-3 text-gray">
            <input  type="text" name="estado_id" class="form-control @error('estado_id') is-invalid @enderror" value="{{old('estado_id',$docente->estado_id ?? '')}}">
