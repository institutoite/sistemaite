...
@section('content')
    <div class="container-fluid">
        <div class="padre-hero p-3 p-md-4 mb-3">
            <div class="d-flex flex-column flex-md-row align-items-md-center justify-content-between">
                <div>
                    ...
                    <small>Consulta rapida de hijos, asistencia, clases y pagos.</small>
                </div>
                <span class="badge mt-2 mt-md-0" style="background: #375f7a; color: #fff;"><i class="fas fa-lock mr-1"></i>Acceso privado</span>
            </div>
        </div>
        ...
        <div class="card">
            <div class="card-header text-white" style="background: #26baa5;">Mis hijos</div>
            <div class="card-body">
                @if($hijos->isEmpty())
                    <p class="text-muted mb-0">No se encontraron hijos asociados a esta cuenta.</p>
                @else
                    <div class="d-block d-md-none">
                        @foreach ($hijos as $hijo)
                            <div class="padre-card">
                                <div class="padre-card-title">{{ $hijo->nombre }} {{ $hijo->apellidop }} {{ $hijo->apellidom }}</div>
                                <div class="padre-card-meta mt-1">Telefono: {{ $hijo->telefono ?? 'Sin telefono' }}</div>
                                ...
                                <div class="mt-3 padre-actions">
                                    <a class="btn btn-sm btn-outline-primary mr-1" href="{{ route('padre.hijos.show', $hijo->id) }}">Ver detalle</a>
                                    <a class="btn btn-sm btn-outline-secondary" href="{{ route('padre.hijos.pdf', $hijo->id) }}">Descargar PDF</a>
                                </div>
                            </div>
                        @endforeach
                    </div>
                            ...
                            <tbody>
                                @foreach ($hijos as $hijo)
                                    <tr>
                                        ...
                                        <td>{{ $hijo->telefono ?? 'Sin telefono' }}</td>
                                        ...
                                        <td class="padre-actions">
                                            <a class="btn btn-sm btn-primary" href="{{ route('padre.hijos.show', $hijo->id) }}">Ver detalle privado</a>
                                            <a class="btn btn-sm btn-secondary" href="{{ route('padre.hijos.pdf', $hijo->id) }}">PDF</a>
                                        </td>
                                    </tr>
                                @endforeach
                            </tbody>
                    ...
                    <p class="text-muted mb-0">Cada consulta valida en backend la relacion padre-hijo para bloquear IDOR y accesos horizontales.</p>
                @endif
            </div>
        </div>
    </div>
